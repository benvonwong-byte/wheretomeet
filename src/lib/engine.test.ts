...
describe('isochrone contours', () => {
  ...
  it('maskField: masked cells become Infinity, land cells untouched, input unmodified', () => {
    ...
    for (let i = 0; i < 30; i++) {
      if (i % 3 === 0) expect(masked[i]).toBe(Infinity);
      else expect(masked[i]).toBe(total[i]);
    }
...
describe('share links', () => {
  ...
  it('a name with the p= delimiters (, ;) does not split into a fake extra person', () => {
    const hash = encodeShare({
      a: { lat: 40.7143, lng: -73.9614 },
      b: { lat: 40.787, lng: -73.9754 },
      modesA: ['walk'],
      ...
      bias: 0,
      daypart: 'midday',
      extra: [{ pt: { lat: 40.7036, lng: -73.9264 }, mode: 'transit', name: 'A; 40.7,-73.9,b,X' }],
    });
    const s = parseShare(hash);
    expect(s.extra?.length).toBe(1); // no injected second person
    expect(s.extra![0].pt.lat).toBeCloseTo(40.7036, 4);
  });
...
describe('venue filter', () => {
  ...
  it('no flags: category filter only', () => {
    ...
    expect(filterVenues(venues, { categories: new Set(['cafe']), ...off })).toHaveLength(3);
  });

  it('veganOnly: only level-2', () => {
    const r = filterVenues(venues, { categories: all, ...off, veganOnly: true });
    expect(r.map((v) => v.id)).toEqual(['a']);
  });

  it('veganFriendly is a disjoint class: excludes fully-vegan places', () => {
    const r = filterVenues(venues, { categories: all, ...off, veganFriendly: true });
